...
class Admin(commands.Cog):
  ...
  async def bal_edit(self, ia: disnake.AppCmdInter, member: disnake.User, amount: int):
    if member.bot:
      ...
      await ia.response.send_message('<аланс изменён.', ephemeral=True)
  ...
  @commands.default_member_permissions(administrator=True)
  async def bal_give(self, ia: disnake.AppCmdInter, member: disnake.User, amount: int):
    if member.bot:
      await ia.response.send_message('Это бот, а не участник. Пожалуйста, выберите кого-нибудь поодушевлённее.',ephemeral=True)
    elif not str(member.id) in self.bot.db['members'].keys():
      await ia.response.send_message('Участника нет в датабазе. Как вы могли такое допустить?!',ephemeral=True)
      ...
      if amount == 0:
        await ia.response.send_message('Это так не работает./nВыберите сумму КРОМЕ НУЛЯ.',ephemeral=True)
      else:
        self.bot.db['members'][str(member.id)]['balance'] += amount
        await ia.response.send_message('баланс изменён.',ephemeral=True)
  ...
  @commands.default_member_permissions(administrator=True)
  async def bal_trans(self, ia: disnake.AppCmdInter, member: disnake.User, amount: int):
    if member.bot:
      await ia.response.send_message('Это бот, а не участник. Пожалуйста, выберите кого-нибудь поодушевлённее.',ephemeral=True)
    elif not str(member.id) in self.bot.db['members'].keys():
      await ia.response.send_message('Участника нет в датабазе. Как вы могли такое допустить?!',ephemeral=True)
    elif member == ia.author:
      await ia.response.send_message('Деньги нельзя переводить самому себе. <:nikobruh:1013910937146773504>',ephemeral=True)
    elif self.bot.db['members'][str(ia.author.id)]['balance'] < amount:
      await ia.response.send_message('Недостаточно средств для перевода. \nНе хватило {0}<:kirieshka:1100873685201588285>'.format(amount-self.bot.db['members'][str(ia.author.id)]['balance']),ephemeral=True)
    else:
      self.bot.db['members'][str(ia.author.id)]['balance'] -= amount
      self.bot.db['members'][str(member.id)]['balance'] += amount
      await ia.response.send_message('Перевод совершён!\n`Ваш баланс:` {0} -> {1}<:kirieshka:1100873685201588285>\n`Баланс `{2}`:` {3} -> {4}<:kirieshka:1100873685201588285>'.format(self.bot.db['members'][str(ia.author.id)]['balance']+amount,self.bot.db['members'][str(ia.author.id)]['balance'],member.mention,self.bot.db['members'][str(member.id)]['balance']-amount,self.bot.db['members'][str(member.id)]['balance']),ephemeral=True)
  ...
  @commands.default_member_permissions(administrator=True)
  @commands.is_owner()
  async def give_bughunter(self, ia: disnake.AppCmdInter, member: disnake.User):
    if member.bot:
      await ia.response.send_message('Это бот, а не участник. Пожалуйста, выберите кого-нибудь поодушевлённее.',ephemeral=True)
    elif not str(member.id) in self.bot.db['members'].keys():
      await ia.response.send_message('Участника нет в датабазе. Как вы могли такое допустить?!', ephemeral=True)
      ...
      if not self.bot.db['members'][str(member.id)]['achievements']['bugHunter']:
        ...
        await ia.response.send_message('Ачивка выдана. Хорошая работа.', ephemeral=True)
        ...
        await ia.response.send_message('Ачивка отнята.', ephemeral=True)

  @give_bughunter.error
  async def give_bughunter_error(self, ia: disnake.AppCmdInter, error: commands.CommandError):
    if isinstance(error, commands.NotOwner):
        await ia.response.send_message("Эта команда доступна только Чичерасту. <a:spongebruh:1068135572230639746>", ephemeral=True)
